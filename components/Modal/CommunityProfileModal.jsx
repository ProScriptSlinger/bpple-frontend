...
const CommunityProfileModal = () => {
  ...
  return (
    <>
      <div
        className={`${
          communityProfileModal && pathName.includes("/community")
            ? "w-[400px]"
            : "w-0"
        ...
        <div className="w-[400px] h-full bg-[#171717] px-[26px] pt-[50px] pb-[50px] flex-col flex">
              ...
              <>
                ...
                <div className="absolute z-10 top-[80px] right-[30px] bg-[#343434] bg-opacity-40 backdrop-blur-[12px] px-[20px] flex flex-col py-[15px] text-[14px] font-thin rounded-[10px] gap-[12px] w-[150px]">
                  ...
                  <button
                    className="text-left inline-flex items-center"
                    onClick={() => {
                      setCommunityProfileModal(false), setDropdown(false);
          ...
          <div className="w-full h-full relative overflow-auto">
            <div className="flex mt-[30px] justify-center w-full">
              <Image
                src={communityProfileInModal?.avatar ?? "/avatar/2.svg"}
                className={` w-[50px] aspect-square rounded-xl 
              ${
                !communityProfileInModal?.avatar && "hidden"
              }   object-cover bg-[#191919] flex items-center justify-center `}
                width={100}
                height={100}
                alt={communityProfileInModal?.name}
                priority={true}
              />
              {communityProfileInModal?.avatar == null && (
                <div
                  className="w-[50px] aspect-square rounded-xl
             bg-[#191919] flex items-center justify-center
              text-[#4C4C4C] text-[22px]"
                >
                  {getNameInitials(communityProfileInModal?.name ?? "B")}
                </div>
              )}
            </div>
            <p className="flex justify-center w-full mt-[20px]">
              {communityProfileInModal.name}
            </p>
            <p className="flex justify-center w-full text-[10px]">
              {communityProfileInModal?.members &&
                ` ${
                  Object.keys(communityProfileInModal?.members).length
                } Members `}
            </p>
